...
def run_case(src_ip='0', dst_ip=None):
    ...
    """
    print 'host_cmd_output_v2'
    start = time.time()
    host_session.host_cmd_output_v2(cmd='dd if=/dev/zero of=/tmp/aa bs=1M count=1000 oflag=direct  ',timeout=300)
    print 'done time : ', time.time() - start

    print 'host_cmd_output'
    start = time.time()
    host_session.host_cmd_output(cmd='dd if=/dev/zero of=/tmp/aa bs=1M count=1000 oflag=direct ')
    print 'done time : ', time.time() - start
    """
    ...
    while 1:
        output = serial.serial_output(max_recv_data=16)
        if re.findall(r'Linux version', output):
            print '@@@@@@@@@@@@ %s !!!!!!!!!!!!!!!' % output
            break
        else:
            pass

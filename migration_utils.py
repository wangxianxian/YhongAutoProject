...
def ping_pong_migration(params, test, cmd, id, src_host_session, src_remote_qmp, dst_remote_qmp, src_ip, src_port,
    ...
    for i in range(1, even_times+1):
        ...
        src_output = src_remote_qmp.qmp_cmd_output(cmd='{"execute":"query-status"}', echo_cmd=False, echo_output=True)
        dst_output = dst_remote_qmp.qmp_cmd_output(cmd='{"execute":"query-status"}', echo_cmd=False, echo_output=True)
        ...
        if re.findall(r'"status": "running"', src_output) and re.findall(r'"status": "inmigrate"', dst_output):
            ...
            src_host_session.boot_remote_guest_v2(ip=dst_ip, cmd=dst_qemu_cmd, vm_alias='dst')
